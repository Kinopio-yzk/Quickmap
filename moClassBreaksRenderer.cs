...
namespace MyMapObjects
{
    public class moClassBreaksRenderer:moRenderer
    {
        ...
        public void RampSize(float StartSize, float EndSize)
        {
            Int32 sBreakCount = _BreakValues.Count;
            if (sBreakCount <= 0)
                return;
            float[] sSizes = new float[sBreakCount];
            if (sBreakCount == 1)
                sSizes[0] = StartSize;
            else
            {
                ...
                float interval = (EndSize - StartSize) / sBreakCount;
                for (Int32 i = 1; i <= sBreakCount - 2; ++i)
                {
                    sSizes[i] = sSizes[0] + i * interval;
                }
            }
